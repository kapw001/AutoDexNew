...
public class IndexLayoutManager extends FrameLayout {
    ...
    private Boolean isHeader(TextView prev, TextView act) {
        ...
        if (prev.getText().length() > 0 && act.getText().length() > 0 && isSameChar(prev.getText().charAt(0), act.getText().charAt(0))) {
    ...
    private char getIndexContext(TextView index) {
        return index.getText().length() > 0 ? index.getText().charAt(0) : ' ';
    }
